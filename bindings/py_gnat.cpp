...
PYBIND11_MODULE(_gnat, m)
{
    py::class_<NearestNeighborsGNATPoint>(m, "NearestNeighborsGNAT")
        .def(py::init<>())
        .def("setDistanceFunction", &NearestNeighborsGNATPoint::setDistanceFunction)
        ...
        .def("nearestK", [](NearestNeighborsGNATPoint &self, const Point &data, std::size_t k) {
            std::vector<Point> nbh;
            self.nearestK(data, k, nbh);
            return nbh;
        })
        .def("nearestR", [](NearestNeighborsGNATPoint &self, const Point &data, double radius) {
            std::vector<Point> nbh;
